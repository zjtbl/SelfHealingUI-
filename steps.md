...
## 0. 2026-04-28 定稿覆盖说明（优先级最高）

本项目从该日期起，执行口径调整为“独立控制面平台 + MCP 解耦”，并采用以下规则：

1. 自愈平台与 Java Playwright 项目独立运行，不与被服务仓库耦合部署。
2. Jenkins 以 webhook 回调平台为主，平台也保留主动触发 Jenkins 的能力。
3. GitLab 仅自动创建分支与 Draft MR，不自动合并主干。
4. 回退策略采用 `revert commit`；若回退冲突，平台仅打标失败并停止自动处理。
5. V1 仅处理 locator 类失败，不处理断言语义变化。
6. 时区统一使用 `Asia/Shanghai`。
7. 一个项目可配置多个 Jenkins Job，并行执行与聚合状态必须可追溯。
...
本文件后续章节保留历史规划参考；若与本节冲突，以本节和 V1 基线文档为准。
